<?php
    ...
    if(!isset($_SESSION['user_id'])){
        ...
        $student_list = $pdo->prepare('SELECT student_name, branch_name, user_name FROM students JOIN branches ON students.branch_id = branches.id where students.id = :id');
        $student_list->execute(array(
            ':id' => $_SESSION['user_id']
        ));
        $row = $student_list->fetch(PDO::FETCH_ASSOC);
        $student_name = $row['student_name'];
        $branch_name = $row['branch_name'];
        $user_name = $row['user_name'];
        $marks = $pdo->prepare('SELECT subject_name, mark FROM marks JOIN subjects ON marks.subject_id = subjects.id WHERE student_id = :id');
        $marks->execute(array(
            ':id' => $_SESSION['user_id']
        ));
        $rows = $marks->fetchAll(PDO::FETCH_ASSOC);
    }
?>
